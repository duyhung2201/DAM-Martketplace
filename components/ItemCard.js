...
export default function ItemCard({ itemData, itemId }) {
	...
	const enableBuy =
		...
		(!itemData.customers_requesting ||
			!itemData.customers_requesting.includes(account)) &&
		(!itemData.customers_granting ||
			!itemData.customers_granting.includes(account));
	const enableCancel =
		account != itemData.owner &&
		(itemData.customers_requesting
			? itemData.customers_requesting.includes(account)
	...
	return (
		<div className="flex flex-col w-11/12 rounded-lg border-gray-50 border-2 shadow-md cursor-pointer hover:shadow-xl">
			...
			<div className="flex flex-col justify-top ml-2">
				<p className="font-semibold text-sm mx-2">
					Price: {itemData.price} ETH
				</p>
				<p
					className="font-semibold text-sm mx-2"
